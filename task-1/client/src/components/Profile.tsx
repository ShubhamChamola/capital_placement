...
export default function Profile({ formData, triggerReq }: Props) {
  const {
    data: {
      attributes: { profile },
    },
  ...
  const { profileQuestions, ...defaultQuestions } = profile;
  ...
  const onCheckBoxChange = (
    ...
    body.data.attributes.profile[key].mandatory = e.target.checked;
  ...
  const onSwitchChange = (checked: boolean, key: string) => {
    ...
    body.data.attributes.profile[key].show = checked;
  ...
  function handleQuestionEdit(id: string, change: any, fromWhere: string) {
    ...
    if (fromWhere === "new") {
      body.data.attributes.profile.profileQuestions.push(change);
      ...
      let currQuestionIndex =
        body.data.attributes.profile.profileQuestions.findIndex(
          (question: any) => question.id === id
        );

      body.data.attributes.profile.profileQuestions[currQuestionIndex] = change;
    }
  ...
  function handleQuestionDelete(id: string, fromWhere: string) {
    ...
    if (fromWhere === "new") {
      ...
      let editedCustomProfileQuestions =
        body.data.attributes.profile.profileQuestions.filter(
          (question: any) => question.id !== id
        );

      body.data.attributes.profile.profileQuestions =
